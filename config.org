...
(use-package pdf-tools
  ...
  :config
  ;; Tell pdf-view-mode which modes are incompatible
  (setq pdf-view-incompatible-modes '(display-line-numbers-mode))
  ;; Install/activate pdf-tools server if needed
  (pdf-tools-install t) ;; the 't' avoids GUI issues
  ;; Better scrolling
  (setq pdf-view-display-size 'fit-page)
  ;; Enable annotation minor mode
  (add-hook 'pdf-view-mode-hook #'pdf-annot-minor-mode)
  ;; Enable text selection minor mode for copying
  (add-hook 'pdf-view-mode-hook
            (lambda ()
              (setq pdf-view-selection-minor-mode t)))

;; ------------------------------
;; Vim-like navigation (Evil)
;; ------------------------------
...
;; ------------------------------
;; Copy-Paste with OCR fallback
;; ------------------------------
(defun my/pdf-copy-text-or-ocr (beg end)
  "Copy PDF text from region or page. Use OCR if no text found."
  (interactive "r")
  (let ((text (if (use-region-p)
                  (pdf-view-active-region-text)
                (pdf-view-current-page-text))))
    (if (and text (not (string-empty-p text)))
        (progn
          (kill-new text)
          (message "PDF text copied!"))
      ;; Fallback OCR
      (when (executable-find "tesseract")
        (let* ((tmp-img (make-temp-file "pdf-ocr" nil ".png"))
               (page-num (pdf-view-current-page))
               (cmd (format "pdftoppm -png -f %d -l %d \"%s\" \"%s\""
                            page-num page-num (buffer-file-name) (file-name-sans-extension tmp-img)))))
          (shell-command cmd)
          (let ((ocr-text (shell-command-to-string (format "tesseract %s - -l eng" tmp-img))))
            (kill-new ocr-text)
            (message "OCR text copied!")))))))

;; Bind copy commands in pdf-view-mode
(with-eval-after-load 'pdf-view
  (define-key pdf-view-mode-map (kbd "C-c C-c") #'my/pdf-copy-text-or-ocr)
  (define-key pdf-view-mode-map (kbd "M-w") #'my/pdf-copy-text-or-ocr))
#+end_src
